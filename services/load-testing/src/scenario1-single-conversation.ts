...
async function fillSurveyForConversation({
    ...
    // Survey presence is captured during setup; absent forms return HTTP 404.
    if (!hasSurvey) {
        return {
            skipped: true,
            success: true,
...
interface ScenarioSetupData {
    conversations: { conversationSlugId: string; hasSurvey: boolean }[];
}
...
export function setup(): ScenarioSetupData {
    ...
    const conversations: ScenarioSetupData["conversations"] = [];
    for (const conversationSlugId of CONVERSATION_SLUG_IDS) {
        const metadataResult = fetchConversationMetadata({
            conversationSlugId,
        });
        ...
        conversations.push({
            conversationSlugId,
            hasSurvey: metadataResult.hasSurvey,
        });
 ...
 */
export default async function ({ conversations }: ScenarioSetupData) {
    ...
    // Survey presence comes from setup; answers remain specific to each guest.
    if (AUTO_FILL_SURVEY) {
        for (const { conversationSlugId, hasSurvey } of conversations) {
