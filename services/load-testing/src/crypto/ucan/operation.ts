...
export async function createDidIfDoesNotExist(
    ...
    const prefixedKey = uniqueId ? `${PREFIXED_KEY}/${uniqueId}` : PREFIXED_KEY;
...
export async function exportKeys(prefixedKey: string): Promise<ExportedKeys> {
    const cryptoStore = await getNodeCryptoStore();
    if (cryptoStore.keystore.exportKeys === undefined) {
        throw new Error("Crypto implementation does not support key export");
    }
...
export async function importKeys(
    ...
    const cryptoStore = await getNodeCryptoStore();
    if (cryptoStore.keystore.importKeys === undefined) {
        throw new Error("Crypto implementation does not support key import");
    }

    const writeName = `${prefixedKey}/write`;
    const exchangeName = `${prefixedKey}/exchange`;
    await cryptoStore.keystore.importKeys(
        writeName,
        exchangeName,
        exportedKeys,
    );
