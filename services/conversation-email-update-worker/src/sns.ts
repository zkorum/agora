...
export async function applySnsInboxItem({
    ...
    const testAttemptPublicId = Object.entries(event.mail.tags ?? {})
        ...
        ?.at(1)
        ?.at(0);
    const parsedTestAttemptPublicId = z.uuid().safeParse(testAttemptPublicId);
    ...
    await db.transaction(async (tx) => {
        ...
        if (correlatedTest !== undefined) {
            ...
            if (suppressions.emailReason !== undefined) {
                await tx
                    ...
                    .values({
                        canonicalEmail: correlatedTest.destinationEmailSnapshot,
                        emailCredentialId:
        ...
        const correlated = (
            await tx
                .select({
                    attemptId: conversationEmailUpdateDeliveryAttemptTable.id,
                    recipientId:
                ...
                .orderBy(
                    validAttemptPublicId === undefined
                        ? asc(conversationEmailUpdateDeliveryAttemptTable.id)
                        : desc(
                              eq(
                                  conversationEmailUpdateDeliveryAttemptTable.publicId,
                                  validAttemptPublicId,
        ...
        if (correlated === undefined) {
            if (item.processingAttemptCount >= MAX_SNS_PROCESSING_ATTEMPTS) {
                await tx
                    .update(conversationEmailUpdateSnsEventInboxTable)
                    .set({
                        status: "dead_letter",
        ...
        if (event.eventType === "Complaint") {
            await tx
                .insert(conversationEmailUpdateUserComplaintSuppressionTable)
                .values({
