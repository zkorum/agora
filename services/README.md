...
- **python-worker-shared/** - Shared Python package for background worker code and generated artifacts
...
### services/python-worker-shared
Shared Python package consumed by Python background workers.

**Contents:** Generated SQLAlchemy models, generated shared constants/types, Valkey queue helpers, retry-state helpers, AI provider helpers, and red-dwarf analysis glue

**Used by:** math-updater, ai-description-retry-worker, description-translation-retry-worker

**Usage:** edit source Python code directly in `services/python-worker-shared/src/`; regenerate `generated_*.py` with `make sync-python-artifacts` after relevant `services/shared` or `services/shared-backend` changes
...
4. **After modifying worker-shared Python logic:** Edit `services/python-worker-shared/src/` directly, then run the affected worker checks
...
services/python-worker-shared (Python worker library)
