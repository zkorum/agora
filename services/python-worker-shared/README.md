...
- Reusable Python implementation lives here under `src/agora_worker_shared/` and is imported as the editable `agora-worker-shared` package by worker services.
- Generated files under `src/agora_worker_shared/generated_*.py` are produced from TypeScript sources in `services/shared` and `services/shared-backend`.
