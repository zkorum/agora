import type { SupportedDisplayLanguageCodes } from "src/shared/languages";

export interface OpinionGroupVisualizationTranslations {
  analysisTabTest: string;
  controls: string;
  clusterCountLabel: string;
  aiLabelsLabel: string;
  withAiLabels: string;
  withoutAiLabels: string;
  distributionLabel: string;
  distributionBalanced: string;
  distributionImbalanced: string;
  ungroupedLabel: string;
  ungroupedNone: string;
  ungroupedSome: string;
  ungroupedMany: string;
  emptySectionsLabel: string;
  emptySectionsNone: string;
  emptySectionsAll: string;
  emptySectionsAgreements: string;
  emptySectionsDisagreements: string;
  emptySectionsDivisive: string;
  clusterCount0: string;
  clusterCount1: string;
  clusterCount2: string;
  clusterCount3: string;
  clusterCount4: string;
  clusterCount5: string;
  clusterCount6: string;
  analysisPreview: string;
  devConversationTitle: string;
  commentPlaceholder: string;
}

export const opinionGroupVisualizationTranslations: Record<
  SupportedDisplayLanguageCodes,
  OpinionGroupVisualizationTranslations
> = {
  en: {
    analysisTabTest: "Analysis Tab Test",
    controls: "Controls",
    clusterCountLabel: "Number of Groups",
    aiLabelsLabel: "LLM Labels",
    withAiLabels: "With LLM Labels",
    withoutAiLabels: "Without LLM Labels (A/B/C fallback)",
    distributionLabel: "Distribution",
    distributionBalanced: "Balanced",
    distributionImbalanced: "Imbalanced",
    ungroupedLabel: "Ungrouped participants",
    ungroupedNone: "None",
    ungroupedSome: "Some (15)",
    ungroupedMany: "Many (120)",
    emptySectionsLabel: "Empty Sections",
    emptySectionsNone: "All sections have data",
    emptySectionsAll: "All sections empty",
    emptySectionsAgreements: "No agreements",
    emptySectionsDisagreements: "No disagreements",
    emptySectionsDivisive: "No divisive",
    clusterCount0: "0 Groups",
    clusterCount1: "1 Group",
    clusterCount2: "2 Groups",
    clusterCount3: "3 Groups",
    clusterCount4: "4 Groups",
    clusterCount5: "5 Groups",
    clusterCount6: "6 Groups",
    analysisPreview: "Analysis Preview",
    devConversationTitle: "Dev analysis preview",
    commentPlaceholder:
      "Select the analysis tab to test the sticky group behavior.",
  },
  ar: {
    analysisTabTest: "اختبار تبويب التحليل",
    controls: "التحكم",
    clusterCountLabel: "عدد المجموعات",
    aiLabelsLabel: "تسميات LLM",
    withAiLabels: "مع تسميات LLM",
    withoutAiLabels: "بدون تسميات (A/B/C)",
    distributionLabel: "التوزيع",
    distributionBalanced: "متوازن",
    distributionImbalanced: "غير متوازن",
    ungroupedLabel: "المشاركون غير المنضمين إلى مجموعات",
    ungroupedNone: "لا أحد",
    ungroupedSome: "بعضهم (15)",
    ungroupedMany: "عدد كبير (120)",
    emptySectionsLabel: "أقسام فارغة",
    emptySectionsNone: "جميع الأقسام تحتوي على بيانات",
    emptySectionsAll: "جميع الأقسام فارغة",
    emptySectionsAgreements: "لا توجد مقترحات معتمدة",
    emptySectionsDisagreements: "لا توجد مقترحات مرفوضة",
    emptySectionsDivisive: "لا توجد مقترحات مثيرة للجدل",
    clusterCount0: "بدون مجموعات",
    clusterCount1: "مجموعة واحدة",
    clusterCount2: "مجموعتان",
    clusterCount3: "3 مجموعات",
    clusterCount4: "4 مجموعات",
    clusterCount5: "5 مجموعات",
    clusterCount6: "6 مجموعات",
    analysisPreview: "معاينة التحليل",
    devConversationTitle: "معاينة تحليل للتطوير",
    commentPlaceholder: "اختر تبويب التحليل لاختبار سلوك تثبيت المجموعة.",
  },
  es: {
    analysisTabTest: "Prueba de Pestaña de Análisis",
    controls: "Controles",
    clusterCountLabel: "Número de Grupos",
    aiLabelsLabel: "Etiquetas LLM",
    withAiLabels: "Con Etiquetas LLM",
    withoutAiLabels: "Sin Etiquetas (A/B/C)",
    distributionLabel: "Distribución",
    distributionBalanced: "Equilibrada",
    distributionImbalanced: "Desequilibrada",
    ungroupedLabel: "Participantes sin grupo",
    ungroupedNone: "Ninguno",
    ungroupedSome: "Algunos (15)",
    ungroupedMany: "Muchos (120)",
    emptySectionsLabel: "Secciones Vacías",
    emptySectionsNone: "Todas las secciones con datos",
    emptySectionsAll: "Todas las secciones vacías",
    emptySectionsAgreements: "Sin proposiciones aprobadas",
    emptySectionsDisagreements: "Sin proposiciones rechazadas",
    emptySectionsDivisive: "Sin proposiciones divisivas",
    clusterCount0: "0 Grupos",
    clusterCount1: "1 Grupo",
    clusterCount2: "2 Grupos",
    clusterCount3: "3 Grupos",
    clusterCount4: "4 Grupos",
    clusterCount5: "5 Grupos",
    clusterCount6: "6 Grupos",
    analysisPreview: "Vista Previa del Análisis",
    devConversationTitle: "Vista previa de análisis de desarrollo",
    commentPlaceholder:
      "Selecciona la pestaña de análisis para probar el comportamiento fijo del grupo.",
  },
  fa: {
    analysisTabTest: "آزمایش زبانه تحلیل",
    controls: "کنترل‌ها",
    clusterCountLabel: "تعداد گروه‌ها",
    aiLabelsLabel: "برچسب‌های LLM",
    withAiLabels: "با برچسب‌های LLM",
    withoutAiLabels: "بدون برچسب (A/B/C)",
    distributionLabel: "توزیع",
    distributionBalanced: "متعادل",
    distributionImbalanced: "نامتعادل",
    ungroupedLabel: "شرکت‌کنندگان بدون گروه",
    ungroupedNone: "هیچ",
    ungroupedSome: "تعدادی (15)",
    ungroupedMany: "تعداد زیاد (120)",
    emptySectionsLabel: "بخش‌های خالی",
    emptySectionsNone: "همه بخش‌ها دارای داده هستند",
    emptySectionsAll: "همه بخش‌ها خالی هستند",
    emptySectionsAgreements: "بدون توافق",
    emptySectionsDisagreements: "بدون مخالفت",
    emptySectionsDivisive: "بدون اختلاف‌نظر",
    clusterCount0: "۰ گروه",
    clusterCount1: "۱ گروه",
    clusterCount2: "۲ گروه",
    clusterCount3: "۳ گروه",
    clusterCount4: "۴ گروه",
    clusterCount5: "۵ گروه",
    clusterCount6: "۶ گروه",
    analysisPreview: "پیش‌نمایش تحلیل",
    devConversationTitle: "پیش‌نمایش تحلیل توسعه",
    commentPlaceholder:
      "برای آزمایش رفتار چسبان گروه، زبانه تحلیل را انتخاب کنید.",
  },
  he: {
    analysisTabTest: "בדיקת לשונית ניתוח",
    controls: "בקרות",
    clusterCountLabel: "מספר קבוצות",
    aiLabelsLabel: "תוויות LLM",
    withAiLabels: "עם תוויות LLM",
    withoutAiLabels: "ללא תוויות (A/B/C)",
    distributionLabel: "התפלגות",
    distributionBalanced: "מאוזן",
    distributionImbalanced: "לא מאוזן",
    ungroupedLabel: "משתתפים ללא קבוצה",
    ungroupedNone: "אין",
    ungroupedSome: "מעט (15)",
    ungroupedMany: "רבים (120)",
    emptySectionsLabel: "סעיפים ריקים",
    emptySectionsNone: "לכל הסעיפים יש נתונים",
    emptySectionsAll: "כל הסעיפים ריקים",
    emptySectionsAgreements: "אין הסכמות",
    emptySectionsDisagreements: "אין חילוקי דעות",
    emptySectionsDivisive: "אין נושאים שנויים במחלוקת",
    clusterCount0: "0 קבוצות",
    clusterCount1: "קבוצה 1",
    clusterCount2: "2 קבוצות",
    clusterCount3: "3 קבוצות",
    clusterCount4: "4 קבוצות",
    clusterCount5: "5 קבוצות",
    clusterCount6: "6 קבוצות",
    analysisPreview: "תצוגה מקדימה של ניתוח",
    devConversationTitle: "תצוגה מקדימה של ניתוח לפיתוח",
    commentPlaceholder:
      "בחרו בלשונית הניתוח כדי לבדוק את התנהגות הקבוצה הדביקה.",
  },
  fr: {
    analysisTabTest: "Test de l'Onglet Analyse",
    controls: "Contrôles",
    clusterCountLabel: "Nombre de Groupes",
    aiLabelsLabel: "Libellés LLM",
    withAiLabels: "Avec Libellés LLM",
    withoutAiLabels: "Sans Libellés (A/B/C)",
    distributionLabel: "Distribution",
    distributionBalanced: "Équilibrée",
    distributionImbalanced: "Déséquilibrée",
    ungroupedLabel: "Participants sans groupe",
    ungroupedNone: "Aucun",
    ungroupedSome: "Quelques-uns (15)",
    ungroupedMany: "Nombreux (120)",
    emptySectionsLabel: "Sections Vides",
    emptySectionsNone: "Toutes les sections remplies",
    emptySectionsAll: "Toutes les sections vides",
    emptySectionsAgreements: "Aucune proposition approuvée",
    emptySectionsDisagreements: "Aucune proposition rejetée",
    emptySectionsDivisive: "Aucune proposition controversée",
    clusterCount0: "0 Groupes",
    clusterCount1: "1 Groupe",
    clusterCount2: "2 Groupes",
    clusterCount3: "3 Groupes",
    clusterCount4: "4 Groupes",
    clusterCount5: "5 Groupes",
    clusterCount6: "6 Groupes",
    analysisPreview: "Aperçu de l'Analyse",
    devConversationTitle: "Aperçu d'analyse de développement",
    commentPlaceholder:
      "Sélectionnez l'onglet analyse pour tester le comportement d'ancrage du groupe.",
  },
  "zh-Hans": {
    analysisTabTest: "分析标签测试",
    controls: "控制",
    clusterCountLabel: "群组数量",
    aiLabelsLabel: "LLM 标签",
    withAiLabels: "带 LLM 标签",
    withoutAiLabels: "无标签 (A/B/C)",
    distributionLabel: "分布",
    distributionBalanced: "均衡",
    distributionImbalanced: "不均衡",
    ungroupedLabel: "未分组参与者",
    ungroupedNone: "无",
    ungroupedSome: "部分 (15)",
    ungroupedMany: "很多 (120)",
    emptySectionsLabel: "空白部分",
    emptySectionsNone: "所有部分都有数据",
    emptySectionsAll: "所有部分为空",
    emptySectionsAgreements: "无通过的意见",
    emptySectionsDisagreements: "无否决的意见",
    emptySectionsDivisive: "无分歧的意见",
    clusterCount0: "无群组",
    clusterCount1: "1 个群组",
    clusterCount2: "2 个群组",
    clusterCount3: "3 个群组",
    clusterCount4: "4 个群组",
    clusterCount5: "5 个群组",
    clusterCount6: "6 个群组",
    analysisPreview: "分析预览",
    devConversationTitle: "开发分析预览",
    commentPlaceholder: "选择分析标签以测试群组粘性行为。",
  },
  "zh-Hant": {
    analysisTabTest: "分析標籤測試",
    controls: "控制",
    clusterCountLabel: "群組數量",
    aiLabelsLabel: "LLM 標籤",
    withAiLabels: "帶 LLM 標籤",
    withoutAiLabels: "無標籤 (A/B/C)",
    distributionLabel: "分布",
    distributionBalanced: "均衡",
    distributionImbalanced: "不均衡",
    ungroupedLabel: "未分組參與者",
    ungroupedNone: "無",
    ungroupedSome: "部分 (15)",
    ungroupedMany: "很多 (120)",
    emptySectionsLabel: "空白部分",
    emptySectionsNone: "所有部分都有資料",
    emptySectionsAll: "所有部分為空",
    emptySectionsAgreements: "無通過的意見",
    emptySectionsDisagreements: "無否決的意見",
    emptySectionsDivisive: "無分歧的意見",
    clusterCount0: "無群組",
    clusterCount1: "1 個群組",
    clusterCount2: "2 個群組",
    clusterCount3: "3 個群組",
    clusterCount4: "4 個群組",
    clusterCount5: "5 個群組",
    clusterCount6: "6 個群組",
    analysisPreview: "分析預覽",
    devConversationTitle: "開發分析預覽",
    commentPlaceholder: "選擇分析標籤以測試群組黏性行為。",
  },
  ja: {
    analysisTabTest: "分析タブテスト",
    controls: "コントロール",
    clusterCountLabel: "グループ数",
    aiLabelsLabel: "LLMラベル",
    withAiLabels: "LLMラベルあり",
    withoutAiLabels: "ラベルなし (A/B/C)",
    distributionLabel: "分布",
    distributionBalanced: "均衡",
    distributionImbalanced: "不均衡",
    ungroupedLabel: "未グループの参加者",
    ungroupedNone: "なし",
    ungroupedSome: "一部 (15)",
    ungroupedMany: "多数 (120)",
    emptySectionsLabel: "空のセクション",
    emptySectionsNone: "全セクションにデータあり",
    emptySectionsAll: "全セクション空",
    emptySectionsAgreements: "承認された意見なし",
    emptySectionsDisagreements: "否決された意見なし",
    emptySectionsDivisive: "分断的な意見なし",
    clusterCount0: "グループなし",
    clusterCount1: "1 グループ",
    clusterCount2: "2 グループ",
    clusterCount3: "3 グループ",
    clusterCount4: "4 グループ",
    clusterCount5: "5 グループ",
    clusterCount6: "6 グループ",
    analysisPreview: "分析プレビュー",
    devConversationTitle: "開発用分析プレビュー",
    commentPlaceholder:
      "グループの固定表示をテストするには、分析タブを選択してください。",
  },
  ky: {
    analysisTabTest: "Анализ өтмөгүн тестирлөө",
    controls: "Башкаруу",
    clusterCountLabel: "Топтордун саны",
    aiLabelsLabel: "LLM энбелгилери",
    withAiLabels: "LLM энбелгилери менен",
    withoutAiLabels: "Энбелгисиз (A/B/C)",
    distributionLabel: "Бөлүштүрүү",
    distributionBalanced: "Теңдештирилген",
    distributionImbalanced: "Теңдешсиз",
    ungroupedLabel: "Топтолбогон катышуучулар",
    ungroupedNone: "Жок",
    ungroupedSome: "Бир аз (15)",
    ungroupedMany: "Көп (120)",
    emptySectionsLabel: "Бош бөлүмдөр",
    emptySectionsNone: "Бардык бөлүмдөрдө маалымат бар",
    emptySectionsAll: "Бардык бөлүмдөр бош",
    emptySectionsAgreements: "Макулдашуулар жок",
    emptySectionsDisagreements: "Макул эместиктер жок",
    emptySectionsDivisive: "Талаштуу пикирлер жок",
    clusterCount0: "0 топ",
    clusterCount1: "1 топ",
    clusterCount2: "2 топ",
    clusterCount3: "3 топ",
    clusterCount4: "4 топ",
    clusterCount5: "5 топ",
    clusterCount6: "6 топ",
    analysisPreview: "Анализди алдын ала көрүү",
    devConversationTitle: "Иштеп чыгуу анализинин алдын ала көрүнүшү",
    commentPlaceholder:
      "Топтун жабышчаак жүрүм-турумун текшерүү үчүн анализ өтмөгүн тандаңыз.",
  },
  ru: {
    analysisTabTest: "Тест вкладки анализа",
    controls: "Управление",
    clusterCountLabel: "Количество групп",
    aiLabelsLabel: "Метки LLM",
    withAiLabels: "С метками LLM",
    withoutAiLabels: "Без меток (A/B/C)",
    distributionLabel: "Распределение",
    distributionBalanced: "Сбалансированное",
    distributionImbalanced: "Несбалансированное",
    ungroupedLabel: "Негруппированные участники",
    ungroupedNone: "Нет",
    ungroupedSome: "Немного (15)",
    ungroupedMany: "Много (120)",
    emptySectionsLabel: "Пустые разделы",
    emptySectionsNone: "Все разделы содержат данные",
    emptySectionsAll: "Все разделы пусты",
    emptySectionsAgreements: "Нет согласий",
    emptySectionsDisagreements: "Нет несогласий",
    emptySectionsDivisive: "Нет спорных",
    clusterCount0: "0 групп",
    clusterCount1: "1 группа",
    clusterCount2: "2 группы",
    clusterCount3: "3 группы",
    clusterCount4: "4 группы",
    clusterCount5: "5 групп",
    clusterCount6: "6 групп",
    analysisPreview: "Предпросмотр анализа",
    devConversationTitle: "Предпросмотр анализа для разработки",
    commentPlaceholder:
      "Выберите вкладку анализа, чтобы проверить закрепление группы.",
  },
};
