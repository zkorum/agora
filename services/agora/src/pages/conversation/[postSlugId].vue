...
function reloadConversationWithUserInteractions(): void {
  console.log(
    "[ConversationPage] Reloading conversation with user interactions"
  );
  // Reload conversation data to fetch user interactions (poll responses, votes, comments)
  refreshConversation(() => {
    console.log(
      "[ConversationPage] Conversation refreshed, now refreshing all tab data"
    );
...
function handleTicketVerifiedInPage(payload: {
  ...
  if (!payload.userIdChanged) {
    console.log(
      "[ConversationPage] Ticket verified, userId unchanged - refreshing data"
    );
    ...
    console.log(
      "[ConversationPage] Ticket verified, userId changed - letting watcher handle refresh"
    );
...
watch(userId, (newUserId, oldUserId) => {
  ...
  if (
    ...
    console.log(
      "[ConversationPage] Account merge detected (userId changed) - reloading data"
    );
