...
> = {
  en: {
    ...
    communityLearnMoreHow: "The ranking combines all participants' comparisons using the Solidago algorithm, an open-source scoring system from the Tournesol project. Each participant's preferences are learned individually, then aggregated into a community ranking. Votes count immediately.",
  ...
  ar: {
    ...
    communityLearnMoreHow: "يجمع الترتيب مقارنات جميع المشاركين باستخدام خوارزمية Solidago، وهو نظام تقييم مفتوح المصدر من مشروع Tournesol. يتم تعلّم تفضيلات كل مشارك على حدة، ثم تُجمع في ترتيب مجتمعي. الأصوات تُحتسب فوراً.",
  ...
  es: {
    ...
    communityLearnMoreHow: "La clasificación combina las comparaciones de todos los participantes usando el algoritmo Solidago, un sistema de puntuación de codigo abierto del proyecto Tournesol. Las preferencias de cada participante se aprenden individualmente y luego se agregan en una clasificacion comunitaria. Los votos cuentan inmediatamente.",
  ...
  fa: {
    ...
    communityLearnMoreHow: "رتبه‌بندی، مقایسه‌های همه شرکت‌کنندگان را با الگوریتم Solidago ترکیب می‌کند، یک سیستم امتیازدهی متن‌باز از پروژه Tournesol. ترجیحات هر شرکت‌کننده به صورت جداگانه یاد گرفته شده و سپس در یک رتبه‌بندی جمعی تجمیع می‌شوند. آرا فوراً محاسبه می‌شوند.",
  ...
  he: {
    ...
    communityLearnMoreHow: "הדירוג משלב את ההשוואות של כל המשתתפים באמצעות אלגוריתם Solidago, מערכת דירוג בקוד פתוח מפרויקט Tournesol. ההעדפות של כל משתתף נלמדות בנפרד ואז מצטברות לדירוג קהילתי. ההצבעות נספרות מיידית.",
  ...
  "zh-Hans": {
    ...
    score: "得分：{score}",
    loadingError: "加载结果失败。",
    communityLearnMoreHow: "排名使用 Solidago 算法综合所有参与者的比较。Solidago 是 Tournesol 项目的开源评分系统。每位参与者的偏好独立学习，然后汇总为社区排名。投票立即生效。",
  ...
  "zh-Hant": {
    ...
    score: "得分：{score}",
    loadingError: "載入結果失敗。",
    communityLearnMoreHow: "排名使用 Solidago 演算法綜合所有參與者的比較。Solidago 是 Tournesol 項目的開源評分系統。每位參與者的偏好獨立學習，然後匯總為社區排名。投票立即生效。",
  ...
  ja: {
    ...
    communityLearnMoreHow: "ランキングは全参加者の比較をSolidagoアルゴリズムで統合します。SolidagoはTournesolプロジェクトのオープンソース評価システムです。各参加者の好みを個別に学習し、コミュニティランキングに集約します。投票は即座に反映されます。",
  ...
  ky: {
    ...
    communityLearnMoreHow: "Рейтинг бардык катышуучулардын салыштырууларын Solidago алгоритми аркылуу бириктирет. Solidago - Tournesol долбоорунун ачык булак баалоо системасы. Ар бир катышуучунун каалоолору өзүнчө үйрөнүлүп, коомдук рейтингге бириктирилет. Добуштар дароо эсепке алынат.",
  ...
  ru: {
    ...
    communityLearnMoreHow: "Рейтинг объединяет сравнения всех участников с помощью алгоритма Solidago -- системы оценки с открытым кодом от проекта Tournesol. Предпочтения каждого участника изучаются индивидуально, затем агрегируются в общий рейтинг. Голоса учитываются сразу.",
