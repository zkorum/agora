...
> = {
  en: {
    ...
    communityLearnMoreHow: "Votes count immediately. Individual preferences are aggregated into a community ranking using Solidago, an open-source algorithm that accounts for trust levels and diverse perspectives.",
  ...
  ar: {
    ...
    communityLearnMoreHow: "الأصوات تُحتسب فوراً. يتم تجميع التفضيلات الفردية في ترتيب مجتمعي باستخدام Solidago، خوارزمية مفتوحة المصدر تراعي مستويات الثقة والوجهات المتنوعة.",
  ...
  es: {
    ...
    communityLearnMoreHow: "Los votos cuentan inmediatamente. Las preferencias individuales se agregan en una clasificacion comunitaria usando Solidago, un algoritmo de codigo abierto que tiene en cuenta los niveles de confianza y las perspectivas diversas.",
  ...
  fa: {
    ...
    communityLearnMoreHow: "آرا فوراً محاسبه می‌شوند. ترجیحات فردی با الگوریتم Solidago، سیستم متن‌باز که سطوح اعتماد و دیدگاه‌های متنوع را در نظر می‌گیرد، در رتبه‌بندی جمعی تجمیع می‌شوند.",
  ...
  he: {
    ...
    retryButton: "נסו שוב",
    communityLearnMoreHow: "ההצבעות נספרות מיידית. העדפות אישיות מצטברות לדירוג קהילתי באמצעות Solidago, אלגוריתם בקוד פתוח שמתחשב ברמות אמון ובנקודות מבט מגוונות.",
  ...
  fr: {
    ...
    communityLearnMoreHow: "Les votes comptent immédiatement. Les préférences individuelles sont agrégées en un classement communautaire via Solidago, un algorithme open source qui tient compte des niveaux de confiance et des perspectives diverses.",
  ...
  "zh-Hans": {
    ...
    loadingError: "加载结果失败。",
    retryButton: "重试",
    communityLearnMoreHow: "投票立即生效。个人偏好通过 Solidago（一种考虑信任级别和多元视角的开源算法）汇总为社区排名。",
  ...
  "zh-Hant": {
    ...
    loadingError: "載入結果失敗。",
    retryButton: "重試",
    communityLearnMoreHow: "投票立即生效。個人偏好透過 Solidago（一種考慮信任級別和多元觀點的開源演算法）匯總為社區排名。",
  ...
  ja: {
    ...
    retryButton: "再試行",
    communityLearnMoreHow: "投票は即座に反映されます。個人の好みはSolidago（信頼レベルと多様な視点を考慮するオープンソースアルゴリズム）でコミュニティランキングに集約されます。",
  ...
  ky: {
    ...
    communityLearnMoreHow: "Добуштар дароо эсепке алынат. Жеке каалоолор Solidago (ишеним деңгээлдерин жана ар түрдүү көз караштарды эсепке алган ачык булак алгоритми) менен коомдук рейтингге бириктирилет.",
  ...
  ru: {
    ...
    communityLearnMoreHow: "Голоса учитываются сразу. Индивидуальные предпочтения агрегируются в общий рейтинг с помощью Solidago, алгоритма с открытым кодом, учитывающего уровни доверия и разнообразие точек зрения.",
