...
export function toPythonEnumMember(value: string): string {
    ...
    const normalizedIdentifier = /[a-z]/.test(identifier)
        ? identifier.toLowerCase()
        : identifier;
    if (pythonKeywords.has(normalizedIdentifier)) {
        return `${normalizedIdentifier}_`;
    }
    return normalizedIdentifier;
}

function assertUniquePythonEnumMembers({
    enumName,
    values,
}: {
    enumName: string;
    ...
    const seenValuesByMember = new Map<string, string>();
    for (const value of values) {
        const member = toPythonEnumMember(value);
        const existingValue = seenValuesByMember.get(member);
        if (existingValue !== undefined) {
            throw new Error(
                `Enum ${enumName} values ${existingValue} and ${value} both generate Python member ${member}`,
            );
        }
        seenValuesByMember.set(member, value);
    }
}
...
export function parseSqlTables(
    ...
    while ((match = tableRegex.exec(sql)) !== null) {
        ...
        for (const line of match[2].split("\n")) {
            ...
            const defaultMatch = /DEFAULT\s+(.+?)(?:\s+NOT NULL|\s*$)/i.exec(
                rest,
            );
...
export function generateSqlAlchemyModels({
    ...
    for (const enumName of usedEnums) {
        ...
        if (!values) continue;
        assertUniquePythonEnumMembers({ enumName, values });
