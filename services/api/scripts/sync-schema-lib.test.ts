...
function getParsedColumns(
    ...
    if (columns === undefined) {
        throw new Error(`Missing parsed table ${tableName}`);
    }
    return columns;
}
...
describe("parseSqlTables", () => {
    ...
    "score" real NOT NULL
        ...
        const cols = getParsedColumns(result, "items");
...
CREATE TABLE "unwanted" ("id" integer NOT NULL);`;
    ...
    "count" integer DEFAULT 0 NOT NULL
        ...
        const cols = getParsedColumns(result, "t");
        expect(cols[0].hasDefault).toBe(true);
...
describe("toPythonEnumMember", () => {
    ...
    it("normalizes mixed-case values", () => {
        expect(toPythonEnumMember("zh-Hant")).toBe("zh_hant");
    });
...
describe("generateSqlAlchemyModels", () => {
    ...
    it("rejects enum values with duplicate Python member names", () => {
        const enums = new Map([["my_status", ["zh-Hant", "zh_hant"]]]);
        const tables = new Map([
            [
                "t",
                [
        ...
        ).toThrow(/both generate Python member zh_hant/);
    });
