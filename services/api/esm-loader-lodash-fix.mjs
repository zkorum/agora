...
export async function resolve(specifier, context, nextResolve) {
    ...
    if (specifier.startsWith("lodash/") && !specifier.endsWith(".js")) {
        ...
        if (context.parentURL && context.parentURL.includes("@pcd/gpc")) {
            ...
            console.log(
                `[ESM Loader] Rewriting lodash import: ${specifier} -> ${fixedSpecifier}`,
            );
    ...
    if (specifier.endsWith(".json") && context.parentURL) {
        if (context.parentURL.includes("@pcd/")) {
            console.log(
                `[ESM Loader] Adding JSON import assertion in resolve: ${specifier}`,
            );
...
export async function load(url, context, nextLoad) {
    ...
    if (
        url.includes("@zk-kit/eddsa-poseidon") &&
        url.includes("node_modules")
    ) {
        console.log(
            `[ESM Loader] Patching @zk-kit/eddsa-poseidon to fix blakejs imports`,
        );
        ...
        if (result.source) {
            ...
            const blakejsImportRegex =
                /import\s*\{([^}]+)\}\s*from\s*['"]blakejs['"]\s*;/g;
