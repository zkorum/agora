...
describe("conversation email update action routes", () => {
    ...
    it("serializes a purpose-limited post-report action and rejects combined opt-out intent", async () => {
        const result = {
            success: true,
            availableAction: {
                action: "unsubscribe",
        ...
        vi.mocked(service.submitReport).mockResolvedValue(
            Dto.conversationEmailUpdateActionReportResponse.parse(result),
        );
        const response = await server.inject({
            method: "POST",
        ...
        expect(response.statusCode).toBe(200);
        expect(
            Dto.conversationEmailUpdateActionReportResponse.parse(
                response.json(),
            ),
        ).toEqual(result);
        expect(response.headers["cache-control"]).toBe("no-store");
        const combined = await server.inject({
            method: "POST",
            url: "/api/v1/conversation/email-update/action/report",
            payload: { token: TOKEN, reason: "spam", unsubscribe: true },
        });
        expect(combined.statusCode).toBe(400);
        expect(service.submitReport).toHaveBeenCalledOnce();
...
describe("conversation email update action service", () => {
    ...
    it.each([
        { scopeKind: "listed_project", participantPreferenceScope: "project" },
        {
            scopeKind: "listed_project",
            participantPreferenceScope: "conversation",
        },
        { scopeKind: "no_project", participantPreferenceScope: "conversation" },
    ] as const)(
        "offers only the authorized $scopeKind/$participantPreferenceScope opt-out after reporting",
        async ({ scopeKind, participantPreferenceScope }) => {
            await seedAction({
                ...
                participantPreferenceScope,
                representedConversationIds:
                    scopeKind === "listed_project" ? [10, 11] : [10],
                authorizedConversationIds: [10],
            });
            await sqlClient`INSERT INTO "project" ("id", "slug") VALUES (2, 'unrelated-project')`;
            await sqlClient`INSERT INTO "conversation" ("id", "project_id", "slug_id") VALUES (12, 2, 'conv0003')`;
            await db
                .insert(conversationEmailUpdateUserConversationPreferenceTable)
                .values(
                    [10, 11, 12].map((conversationId) => ({
                        userId: USER_ID,
                        conversationId,
                        enabled: true,
                        choiceAt: new Date("2025-01-01"),
                        choiceSource: "settings" as const,
                    })),
                );
            const initialPreferences = await db
                .select()
                .from(conversationEmailUpdateUserConversationPreferenceTable);
            expect(await service.resolve({ token: TOKEN })).toMatchObject({
                success: true,
            ...
            if (!response.success || response.availableAction === undefined) {
                throw new Error(
                    "Participant report must offer an unsubscribe action",
                );
            }
            const offered = response.availableAction;
            expect(offered.action).toBe("unsubscribe");
            expect(offered.token).not.toBe(TOKEN);
            expect(offered.scope).toMatchObject({
                kind:
            ...
            expect(
                await service.resolve({ token: offered.token }),
            ).toMatchObject({
                success: true,
                action:
                    participantPreferenceScope === "project"
            ...
            for (const token of [TOKEN, offered.token]) {
                expect(
                    await service.manageOptOut({
                        token,
                        target: {
            ...
            const tokens = await db
                ...
                .where(
                    eq(
                        ...
                        hashConversationEmailUpdateActionToken(offered.token),
                    ),
                );
            expect(tokens).toHaveLength(1);
            expect(tokens.at(0)?.attemptPublicId).toBe(
                "00000000-0000-4000-8000-000000000001",
            );
            const expiresAt = tokens.at(0)?.expiresAt;
            ...
            for (let replay = 0; replay < 2; replay += 1) {
                expect(
                    await service.unsubscribe({ token: offered.token }),
            ...
            expect(
                ...
                {
                    conversationId: 11,
                    enabled: participantPreferenceScope !== "project",
                },
                { conversationId: 12, enabled: true },
            ]);
            const projectPreferences = await db
                .select()
                .from(conversationEmailUpdateUserProjectPreferenceTable);
            expect(projectPreferences).toMatchObject(
                participantPreferenceScope === "project"
                    ? [{ userId: USER_ID, projectId: 1, enabled: false }]
                    : [],
            );
            expect(
                await service.submitReport({
            ...
            expect(
                ...
                { recipientId: 40n, reason: "spam", details: "First report" },
            ]);
        },
    );

    it("caps the offered capability expiry and refuses expired or invalid report/unsubscribe tokens", async () => {
        await seedAction({
            token: TOKEN,
            action: "report",
            scopeKind: "no_project",
        ...
        const expiry = new Date(Math.floor(Date.now() / 1000) * 1000 + 60_000);
        await db
            .update(conversationEmailUpdateActionTokenTable)
            .set({ expiresAt: expiry });
        const response = await service.submitReport({
            token: TOKEN,
            reason: "spam",
        });
        if (!response.success || response.availableAction === undefined)
            throw new Error("Expected participant action");
        const offeredToken = response.availableAction.token;
        const offeredRows = await db
            ...
            .where(
                eq(
                    conversationEmailUpdateActionTokenTable.tokenHash,
                    hashConversationEmailUpdateActionToken(offeredToken),
                ),
            );
        expect(offeredRows.at(0)?.expiresAt).toEqual(expiry);
        await sqlClient`UPDATE "conversation_email_update_action_token" SET "created_at" = '1999-01-01', "expires_at" = '2000-01-01'`;
        for (const token of [TOKEN, OTHER_TOKEN, offeredToken]) {
            expect(await service.resolve({ token })).toEqual({
                success: false,
                reason: "unavailable",
            });
            expect(
                await service.submitReport({ token, reason: "abuse" }),
            ).toEqual({ success: false, reason: "unavailable" });
        ...
        expect(
            await db.select().from(conversationEmailUpdateActionTokenTable),
        ).toHaveLength(2);
    });

    it("rejects a legacy owner manage token", async () => {
        await seedAction({
            token: TOKEN,
            action: "manage_preferences",
            scopeKind: "listed_project",
    ...
    it("rejects a legacy owner project-unsubscribe token", async () => {
        await seedAction({
            token: TOKEN,
            action: "unsubscribe_project",
            scopeKind: "listed_project",
