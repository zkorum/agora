...
CREATE TYPE "public"."premium_feature" AS ENUM('survey', 'event_ticket', 'analysis_variants');--> statement-breakpoint
...
CREATE TABLE "analysis_work_state" (
	...
	"persisted_analysis_snapshot_id" integer,
...
CREATE TABLE "opinion_group_description_locale_status" (
	...
	"ai_generation_expected" boolean DEFAULT false NOT NULL,
	"translation_expected" boolean DEFAULT false NOT NULL,
	"attempt_count" integer DEFAULT 0 NOT NULL,
	"next_run_at" timestamp (0),
	"lease_owner" varchar(100),
	"lease_token" varchar(100),
...
ALTER TABLE "analysis_work_state" ADD CONSTRAINT "analysis_work_state_persisted_analysis_snapshot_id_analysis_snapshot_id_fk" FOREIGN KEY ("persisted_analysis_snapshot_id") REFERENCES "public"."analysis_snapshot"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "analysis_work_state_due_idx" ON "analysis_work_state" USING btree ("next_run_at","conversation_id") WHERE ("analysis_work_state"."running_data_generation" is null AND "analysis_work_state"."next_run_at" is not null);--> statement-breakpoint
CREATE INDEX "analysis_work_state_lease_expiry_idx" ON "analysis_work_state" USING btree ("lease_expires_at") WHERE "analysis_work_state"."running_data_generation" is not null;--> statement-breakpoint
...
CREATE INDEX "conversation_view_snapshot_latest_idx" ON "conversation_view_snapshot" USING btree ("conversation_id","created_at" DESC NULLS LAST,"id" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "conversation_view_snapshot_latest_active_idx" ON "conversation_view_snapshot" USING btree ("conversation_id","created_at" DESC NULLS LAST,"id" DESC NULLS LAST) WHERE "conversation_view_snapshot"."activated_at" is not null;--> statement-breakpoint
CREATE INDEX "conversation_view_snapshot_latest_spec_active_idx" ON "conversation_view_snapshot" USING btree ("conversation_id","opinion_group_spec_id","created_at" DESC NULLS LAST,"id" DESC NULLS LAST) WHERE "conversation_view_snapshot"."activated_at" is not null;--> statement-breakpoint
CREATE INDEX "conversation_view_snapshot_analysis_snapshot_idx" ON "conversation_view_snapshot" USING btree ("analysis_snapshot_id") WHERE "conversation_view_snapshot"."analysis_snapshot_id" is not null;--> statement-breakpoint
...
CREATE INDEX "opinion_group_description_translation_work_lease_expiry_idx" ON "opinion_group_description_translation_work" USING btree ("lease_expires_at") WHERE "opinion_group_description_translation_work"."lease_token" is not null;--> statement-breakpoint
CREATE INDEX "opinion_group_lineage_description_work_due_idx" ON "opinion_group_lineage_description_work" USING btree ("next_run_at") WHERE ("opinion_group_lineage_description_work"."lease_token" is null AND "opinion_group_lineage_description_work"."next_run_at" is not null);--> statement-breakpoint
CREATE INDEX "opinion_group_lineage_description_work_lease_expiry_idx" ON "opinion_group_lineage_description_work" USING btree ("lease_expires_at") WHERE "opinion_group_lineage_description_work"."lease_token" is not null;--> statement-breakpoint
